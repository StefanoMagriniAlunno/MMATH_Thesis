 ...
 * @brief main source file of the synthesis project. A python script call
...
/**
 * @brief This function read input from Python and call csynthesis function
 * from synthesis.h
 *
 * @param self
 * @param args :
 ...
 * @exception ValueError : if the size of tiles or number of threads is not
 * valid
 */
static PyObject *
wrapper (PyObject *self, PyObject *args)
{
  ...
  const char *in_dset_path = NULL, *out_dset_path = NULL, *file_path = NULL,
             *log_path = NULL;
  int n_threads = 0, n_tiles = 0;
  if (!PyArg_ParseTuple (args, "sssiis", &in_dset_path, &out_dset_path,
                         &file_path, &n_tiles, &n_threads, &log_path))
    {
      PyErr_SetString (PyExc_SyntaxError,
                       "Required 3 strings in input and an integer.");
      return NULL;
    }
  if (!(n_tiles > 0))
    {
      PyErr_Format (PyExc_ValueError, "Size of tiles is not valid. Passed %d",
                    n_tiles);
      return NULL;
    }
  if (!(n_threads > 0))
    {
      PyErr_Format (PyExc_ValueError,
                    "Number of threads is not valid. Passed %d", n_threads);
      return NULL;
    }
  ...
  switch (ret)
    ...
    case LOG_ERROR:
      PyErr_Format (PyExc_Exception,
                    "log file error, please check %s for more details",
                    log_path);
      Py_RETURN_NONE;
    case IO_ERROR:
      PyErr_Format (PyExc_OSError,
                    "an error occurred during IO operations, please check %s "
                    "for more details",
                    log_path);
      Py_RETURN_NONE;
    case VALUE_ERROR:
      PyErr_Format (PyExc_ValueError,
                    "invalid input values, please check %s for more details",
                    log_path);
      Py_RETURN_NONE;
    case MEMORY_ERROR:
      PyErr_Format (
          PyExc_MemoryError,
          "memory allocation error, please check %s for more details",
          log_path);
      Py_RETURN_NONE;
    }
...
static struct PyModuleDef libsynthesis = {
  ...
  -1, // Permette il caricamento di più istanze dello stesso modulo in uno
