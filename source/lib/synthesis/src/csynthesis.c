...
#pragma omp parallel for schedule(static) num_threads(n_threads)
  for (unsigned row = 0; row < height_short; ++row)
    for (unsigned col = 0; col < width_short;
         ++col) // fisso il pixel che voglio inserire
      for (unsigned i = 0; i < n_tiles; ++i)
        for (unsigned j = 0; j < n_tiles; ++j) // identifico dove inserirlo
          {
            long unsigned index_out
                = ((row * width_short + col) * n_tiles + i) * n_tiles
                  + j; // [row-i, col-j, i,j]
         ...
         const unsigned n_tiles, const unsigned n_threads, FILE *log_file)
{
  ...
  if (in_file == NULL)
    {
      fprintf (log_file,
               "in file %s at line %d : CRITICAL fopen error, input_path=%s\n",
  ...
  if (width < n_tiles && height < n_tiles)
    {
      fclose (in_file);
      fprintf (log_file,
               "in file %s at line %d : CRITICAL value error, n_tiles=%u\n",
  ...
  if (byte_board == NULL)
    {
      fclose (in_file);
      fprintf (log_file,
               "in file %s at line %d : CRITICAL calloc error, size=%lu\n",
               __FILE__, __LINE__, size);
      fflush (log_file);
      return MEMORY_ERROR;
    }
  if (fread (byte_board, sizeof (unsigned char), size, in_file)
  ...
  cells_size = (long unsigned)n_tiles * n_tiles * (height - n_tiles + 1)
               * (width - n_tiles + 1);
  ...
  if (cells == NULL)
    {
      ...
      fprintf (
          log_file,
          "in file %s at line %d : CRITICAL calloc error, cells_size=%lu\n",
  ...
  fprintf (
      log_file,
      "in file %s at line %d : DEBUG starting synthetizer with %u threads\n",
  ...
  if (out_file == NULL)
    {
      ...
      free (cells);
      fprintf (log_file,
               "in file %s at line %d : CRITICAL fopen error, out_path=%s\n",
               __FILE__, __LINE__, out_path);
      fflush (log_file);
      return IO_ERROR;
    }
  if (fwrite (cells, sizeof (unsigned char), cells_size, out_file)
            ...
            const unsigned n_threads, const char *const log_file_path)
{
  ...
  {
    char path[PATH_MAX];
    while (fgets (path, sizeof (path), list_file) != NULL
           && error_detected == SUCCESS)
      {
        ...
        {
          int chars_written = fprintf (
              log_file, "in file %s at line %d : INFO processing %s\n",
        ...
        if (ret != SUCCESS)
          {
            int chars_written = fprintf (
                log_file,
                "in file %s at line %d : CRITICAL _reader error, code %d\n",
