 ...
 * @brief main source file of the fcm project. A python script call functions
 * of this file.
 *
 * @date 2024-07-31
 *
...
/**
 * @brief This function reads input from Python and calls the cxxfcm function
 * from fcm.h
 *
 * @param self
 * @param args :
...
{
  PyObject *
  fcmwrapper (PyObject *self, PyObject *args)
  {
    if (self == NULL)
      {
    ...
    if (dimension <= 0)
      {
        PyErr_Format (PyExc_ValueError,
                      "Size of data points is not valid. Passed %d",
                      dimension);
        return NULL;
      }
    ...
    switch (ret)
      ...
      case IO_ERROR:
        PyErr_Format (
            PyExc_OSError,
            "an error occurred during IO operations, please check %s for "
            "more details",
            log_path);
        Py_RETURN_NONE;
      case DEVICE_ERROR:
        PyErr_Format (
            PyExc_RuntimeError,
            "an error occurred during device operations, please check %s "
            "for more details",
            log_path);
        Py_RETURN_NONE;
      }
